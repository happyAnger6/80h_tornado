...
from model.sign import SignModel
...
class AjaxHandler(BaseHandler):
	...
	@gen.coroutine
	def _sign_action(self):  #签到控制
		user_info = self.current_user
		if not user_info or user_info["username"] == "guest": #必须是登陆用户才能签到
			self.json("fail", "please sign in first")
		...
		sign_info = yield self.db.sign.find_one(sign) #查询是否已经签到
		if sign_info:
			self.json("fail", "you have already signed today")

		model = SignModel()
		if not model(sign):
			self.json("fail",model.error_msg)

		result = yield self.db.sign.insert(sign) #
		yield self.db.member.update(user,{"$inc":{"money":1}})
		self.json("success",user['money']+1)
