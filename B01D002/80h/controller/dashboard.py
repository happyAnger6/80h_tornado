...
class AdminHandler(BaseHandler):
	...
	@tornado.web.asynchronous
	@gen.coroutine
	def _view_newgame(self, arg): #添加一个新游戏
		self.render("newgame.htm")

	@tornado.web.asynchronous
	@gen.coroutine
	def _view_gamedetail(self, arg): #修改某一个游戏
		id = arg
		#print("id",id)
		game = yield self.db.games.find_one({
			"_id": ObjectId(id)
		})
		if not game:
			self.custom_error("不存在这个游戏")
		self.render("gamedetail.htm", game = game)